...
(filter_results.html was initially used before replaced by filter_grid.html)
(all_movies_admin.html is used to check more ids to ensure CRUD accuracy)

### Features Left to Implement
Feature ideas would be :
- to display movie images / thumbnails
- allow user to upload movie images / thumbnails
- allow multiple words to be searcheable
...
- MySQL / pymysql / phpPHPmyadmin

- Canva (www.canva.com) : Canva was used to design and create the website logo

- Templates Point (www.templatespoint.net) : The frontend HTML CSS JS is largely obtained from a free responsive sample movie template (FlixGo) provided by Templates Point, and stripped and modified, in order to save time in designing the backbone of the look and feel.

- Bootstrap (getbootstrap.com) : Bootstrap is used to enable mobile responsive webpages

- jQuery (jquery.com) : This project uses **JQuery** to simplify DOM manipulation, which is also used by templates point and bootstrap themselves
...
2. Filter By:
    ...
    5. Go to "Full Database" page to check all details retrieved and verified that filtered results shows the same information as per database
...
7. Different Browsers:
    ...
    2. Only internet explorer shows that the table header position: sticky isn't working. This is in full database and search results page.
    3. for Mobile Browsers, The tables in full database and search results page suffers on my personal mobile where 
